...
export class PaymentService {
  ...
  static async createPayment(userId: string, orderId: string) {
    ...
    // 4. Duplicate payment protection: check if there is already a CAPTURED payment
    ...
    // 6. Create payment record in database and update order status in a transaction
  ...
  static async verifyPayment(
    ...
    // 2. Fetch payment and order details
    ...
    // 4. State validation: check if payment is already captured
    if (payment.status === 'CAPTURED') {
      return { success: true, paymentId: payment.id, orderId: payment.order_id, message: 'Already verified' };
    }

    // 5. Fetch payment details from Razorpay to verify capture and amount consistency
    let rzpPayment;
    try {
  ...
  static async processWebhook(rawBody: Buffer, signature: string) {
    ...
    const paymentRes = await pool.query('SELECT * FROM payments WHERE razorpay_order_id = $1', [razorpayOrderId]);
    if (paymentRes.rows.length === 0) {
